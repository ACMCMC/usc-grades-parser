...
no_presentados = list(map(lambda x: int(x)-1, re.findall("\d+(?=\s.+Non\sPresentado)", pdf)))
#list(map(lambda x: nombres.pop(int(x)-1), no_presentados))
nombres = list(map(lambda x: x[1], filter(lambda tupla: True if tupla[0] not in no_presentados else False, enumerate(nombres))))
apellidos = list(map(lambda x: x[1], filter(lambda tupla: True if tupla[0] not in no_presentados else False, enumerate(apellidos))))
